...
export async function handleProratedBilling(env: any, customerId: number, newSubscriptionPlanId: number) {
  try {
    ...
    if (totalAmount > 0) {
      // Customer needs to pay more (upgrade)
      invoiceResult = await env.DB.prepare(
        "INSERT INTO invoice (customer_id, amount, due_date, payment_status) VALUES (?, ?, ?, ?)"
      ).bind(customerId, totalAmount, today.toISOString().split('T')[0], 'pending').run();

      console.log(`Customer ${customerId} needs to pay an additional ${totalAmount} for the upgrade.`);
    } else {
      // Customer should get a refund (downgrade)
      const refundAmount = Math.abs(totalAmount);
      invoiceResult = await env.DB.prepare(
        "INSERT INTO invoice (customer_id, amount, due_date, payment_status) VALUES (?, ?, ?, ?)"
      ).bind(customerId, -refundAmount, today.toISOString().split('T')[0], 'refunded').run();
    }

    // Update the customer's subscription to the new plan
    await env.DB.prepare(
      "UPDATE customer SET subscription_plan_id = ?, next_billing_date = ? WHERE id = ?"
    ).bind(newSubscriptionPlanId, calculateNextBillingDate(newSubscriptionPlan.billing_cycle), customerId).run();

    console.log(`Prorated billing handled for customer ${customerId} with new plan ${newSubscriptionPlanId}`);
...
export async function handleGenerateInvoice(customerId: any, env: any, subscription_plan_id: string, eventType: string) {
  try {
